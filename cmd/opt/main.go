...
var (
	...
	printSkipped = flag.Bool("print-skipped", false, "Print a list of skipped characters. Default false")
	gcsimExport  = flag.Bool("gcsim-export", false, "Export to gcsim config string. Default false")
)
...
func main() {
	...
	done := make(map[good.CharacterKey]bool)
	...
	for _, char := range prio {
		...
		done[cfg.Character.Key] = true

		if *gcsimExport {
			writeConfigToFile(cfg, &opt)
		}

	}
	if *printSkipped {
		fmt.Println("Following characters were skipped:")
		keys := make([]good.CharacterKey, 0, len(done))
		for k, v := range done {
			if !v {
				keys = append(keys, k)
			}
